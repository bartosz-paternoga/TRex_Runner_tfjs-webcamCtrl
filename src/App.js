...
main = () => {
  ...
  if (obstacle){
    ...
    const standarize = (inpt) => {
          inpt = tf.tensor(inpt);
       
          return inpt.sub(mean0).div(variance0.pow(0.5)); 
       };
    
    
    const xSinglePred = standarize(([[obstacle.xPos, obstacle.yPos, obstacle.width,	spd]]));
    
    let spr;
    const SinglePredictiction = async () => {
    const sp = await model.predict(xSinglePred);
    spr = sp.round().get(0,0)
    console.log("Single predictiction:",spr);
    if (spr === 1) {
        simulateKey(38); // arrow up
      }
    };
...
// oXpos.push(obstacle.xPos);
// oYpos.push(obstacle.yPos);
// oWidth.push(obstacle.width);
// spdArray.push(spd);
// jmpArray.push(jmp.length);

// let ys=[];

// for (let i = 0; i < jmpArray.length; i++) { 
//   if ((jmpArray[i] - jmpArray[i-1]) === 0){
//    ys.push(0)
//   } else {ys.push(1)}
// };
// ys[0] = 0;
...
// for (let i = 0; i < oXpos.length; i++) { 
// xs.push([oXpos[i],oYpos[i],oWidth[i],spdArray[i],ys[i]])
// };

// // console.log("XSSSSS",xs);
// console.log("YSSSS",ys.length);

// function download_csv() {
//   var csv = 'xPos,yPos,width,speed,jump\n';
//   xs.forEach(function(row) {
//           csv += row.join(',');
//           csv += "\n";
//   });
...
// if(oXpos.length===30000){
//   //download_csv();
// }


// console.log("JMP:",jmpArray,"|","obstacle.width|xPos|yPos",oWidth,"|",oXpos,"|",oYpos , "|","Speed",spdArray )

// console.log("jmpArrayjmpArrayjmpArray",jmpArray)
// console.log("jmpMjmpMjmpM",jmpM)

       // console.log("JMP:",jmp.length,"|","obstacle.width|xPos|yPos",obstacle.width,"|",obstacle.xPos,"|",obstacle.yPos , "|","Speed",spd )
  } 
